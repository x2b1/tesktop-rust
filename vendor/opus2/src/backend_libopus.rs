...
impl Encoder {
	...
	/// Enables Opus 1.6's experimental 96 kHz quality-extension layer.
	///
	/// The bundled libopus must be built with `ENABLE_QEXT`, and the encoder
	/// must be created at 96 kHz. Standard Opus decoders can still decode the
	/// compatible base layer.
	pub fn set_qext(&mut self, enabled: bool) -> Result<()> {
		let code = unsafe {
			ffi::opus_encoder_ctl(
				self.ptr,
				ffi::OPUS_SET_QEXT_REQUEST as c_int,
				if enabled { 1 } else { 0 },
			)
		};
		if code < 0 {
			Err(Error::from_code("opus_encoder_ctl(OPUS_SET_QEXT)", code))
		} else {
			Ok(())
		}
	}
...
encoder_dtx_ctls!(Encoder, opus_encoder_ctl);
...
encoder_dtx_ctls!(MSEncoder, opus_multistream_encoder_ctl);
