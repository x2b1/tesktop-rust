...
| AutoDeleteDMs | A conversation that has gone quiet is left after the days you set, never a group unless you ask, never one with too little in it, and nothing at all until you set the delay | The original's other rules, which are about who counts as a direct message rather than about the delay |
| StartupTimings | How long the ports took to come up, as the frames it took before anything asked of them | The original's phase-by-phase page, which reads the client's own internal phases |
| RobloxFilter | The content filter, with all 24 rules and 158 word-level replacements taken from the original's own file rather than retyped, blocking by default, censoring with the spaces kept, or writing the rule's own word, in both directions, with a strict table for stricter detection | Reading the modifier keys, so the original's hold-to-override is gone; strict mode may catch more than you want |
| AutoDeleter | Your own messages are taken back after the delay you set, in seconds, minutes or hours, only in the conversations you name, never a message holding a word you keep, and nothing at all until the delay is set | The four deletion modes, which change how the message goes rather than when; the app deletes the ordinary way |
